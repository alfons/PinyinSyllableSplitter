...
function splitPinyin(text, resuffixnumbers = true) {
    ...
    // Step 3: re-suffix numbers for number marked pinyin
    if (resuffixnumbers) {
    ...
    // Step 4: spellcheck correction for overfetched diacritic cases
    // e.g. "jìniàn" → ["jìn", "iàn"] → ["jì", "niàn"]
    const fixed = [];
    for (let i = 0; i < result.length; i++) {
        let curr = result[i];
        ...
        if (!sylSet.has(bare_curr) && i > 0) {
            ...
            // Only apply the correction if the PREVIOUS syllable has NO trailing number
            if (/[1-5]$/.test(bare_prev)) {
                fixed.push(curr);
                continue;
            }
            ...
            if (bare_prev.length > 1) {
                ...
                // check if the new versions are both in the dictionary
                // current syllable might have a number, remove for dictionary check
                if (sylSet.has(new_bare_prev) && sylSet.has(new_bare_curr.replace(/[1-5]$/, ''))) {
                    // Apply fix
